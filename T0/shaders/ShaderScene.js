class ShaderScene extends CGFscene 
{
    ...
    init(application) 
    {
		...
		this.objects= [new Teapot(this), new Plane(this, 50)];

		// Object interface variables
		this.objectList = { 'Teapot': 0, 'Plane': 1 }
		...
		// set the scene update period to 20 ms per frame
		this.setUpdatePeriod(20);
	};


	// configure cameras
	initCameras() { this.camera = new CGFcamera(0.4, 0.1, 500, vec3.fromValues(20, 20, 100), vec3.fromValues(0, 0, 0)); };
	...
	onSelectedShaderChanged(v){
        ...
        this.onScaleFactorChanged(this.scaleFactor);    // update scale factor
	}

    
	// called when a new object is selected
    // update wireframe mode when the object changes
	onSelectedObjectChanged(v) { this.onWireframeChanged(this.wireframe); }
    ...
    onScaleFactorChanged(v) { this.testShaders[this.selectedExampleShader].setUniformsValues({ normScale: this.scaleFactor }); }

	// called periodically (as per setUpdatePeriod() in init())
    // only shader 6 is using time factor
	update(t) { if (this.selectedExampleShader == 6) this.testShaders[6].setUniformsValues({ timeFactor: t / 100 % 1000 }); }
    ...
    display()   // main display function
    {
        ...
        this.lights[0].update();    // Update all lights used
        this.axis.display();        // Draw axis
        
        this.appearance.apply();    // aplly main appearance (including texture in default texture unit 0)
        this.setActiveShader(this.testShaders[this.selectedExampleShader]); // activate selected shader
        
        this.pushMatrix();
        this.texture2.bind(1);      // bind additional texture to texture unit 1
